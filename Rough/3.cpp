...
inline bool compbyss(pair<pll, ll> x, pair<pll, ll> y) { return (x.ff.ss - x.ff.ff) > (y.ff.ss-y.ff.ff); }
...
class DSU {
private :
    vector<int> par, size, edge, rank, dist;
public :
    DSU(int n) {
    ...
    int componentCount(int n) {
        int count = 0;
        for(int i=1;i<=n;i++) { if(find(i) == i) count++; }
        return count;
    }
    bool isSame(int x, int y) { return find(x) == find(y); }
    int getSize(int x) { return size[find(x)]; }
    int getEdgeCount(int x) { return edge[find(x)]; }
};
...
void solve()
{
    ...
    sort(all(edges),compbyss);
    DSU ds(high);
    vll ans;
    for(auto &e: edges){
        if(ds.find(e.ff.ff)!=ds.find(e.ff.ss)){
            ans.push_back(e.ss);
            ds.unitebysize(e.ff.ff, e.ff.ss);
        }
    }
    ll z=ans.size();
    o1(z);
